...
export class SerialSimComponent implements OnInit {
  ...
  async onNextStep(nameSelect?) {
    ...
    if (this.enableScanCode) {
      ...
      if (this.isCropImage && this.isPhotoTaken && !this.isScanSuccess) {
        const hints = new Map();
        const enabledFormats = [BarcodeFormat.CODE_128];
        hints.set(DecodeHintType.POSSIBLE_FORMATS, enabledFormats);
        const codeReader = new BrowserMultiFormatReader(hints);
        codeReader
          .decodeFromImage(undefined, this.imgBarCode)
          .then((result: any) => {
            console.log(result);
            if (result && result.text) {
              ...
              this.alertService.showError(
                "Chưa có kết quả serial SIM, vui lòng thử lại hoặc nhập tay"
              );
              return;
            }
          })
    ...
    if (
      this.currentTask?.sub_action != TelecomTaskSubAction.BUY_ESIM &&
      (!this.serialSim || !this.imageFront)
    ) {
      this.alertService.showError("Vui lòng nhập serial SIM và tải ảnh SIM");
      return;
    }
    if (
      (this.enableScanCode && this.isScanSuccess) ||
      this.serialSim ||
      this.currentTask?.sub_action == TelecomTaskSubAction.BUY_ESIM
    ) {
      this.submitData();
  ...
  submitData() {
    if (this.enableScanCode) {
      ...
      if (videoCamera && !this.isManualInputCode) {
        ...
        this.imageSim = this.dataURLtoFile(
      ...
      this.imageSim = this.imageFront
        ? this.dataURLtoFile(
            "data:image/png;base64," + this.imageFront,
            `${this.serialSim}.png`
          )
        : null;
    }
    ...
    this.telecomService.taskAddKit(formData).subscribe(
      (res) => {
        if (!res.status) {
  ...
  async initCamera() {
    ...
    const codeReader = new BrowserMultiFormatReader(hints);
    codeReader
      .decodeOnceFromVideoDevice(undefined, "video-scan")
      .then((result: any) => {
        console.log(result);
        if (result && result.text) {
          this.serialSim = result.text;
  ...
  onSelectFile(event) {
    this.convertFile(event.target.files[0]).subscribe(async (base64) => {
      ...
      if (this.isUseOCR) {
        ...
        try {
          const resultOCR = await this.telecomService
            .taskScanSerial(dataOcr)
            .toPromise();
  ...
  ngOnChanges(): void {
    if (this.selectedPackage) {
      ...
      this.currentTask = JSON.parse(
        localStorage.getItem(ObjectLocalStorage.CURRENT_TASK)
      );
      console.log(this.currentTask);
      if(this.currentTask?.msisdns[0]?.is_gsim == 1){
