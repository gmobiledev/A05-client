...
export class TaskComponent implements OnInit {
  ...
  async onCreateTask() {
    ...
    } else if(this.currentService == ServiceCode.ADD_DATA_BALANCE || this.currentService == ServiceCode.ADD_MONEY_BALANCE) {
  ...
  onCreateBalanceService() {
    ...
    this.taskService.addBalance(dataPost).subscribe(res => {
      this.alertService.showSuccess(res.message);
      this.modalClose();
  ...
  getData(): void {
    ...
    this.packageService.getAll({type: 1, category: 'INBOUD_TOUR'}).subscribe(res => {
